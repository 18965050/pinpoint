...
:start_component
  rem set MAVEN=..\..\mvnw.cmd
  set MAVEN=%MVN_HOME%\bin\mvn.cmd
...
:start_process
  ...
  set MAVEN_OPTS=%MAVEN_OPTS% -agentlib:jdwp=transport=dt_socket,address=%DEBUG_PORT%,server=y,suspend=n
  ...
  for /f "tokens=6 delims=," %%i in ('wmic process where Name^="java.exe" get ProcessId^,CommandLine /Format:csv ^| findstr "%IDENTIFIER%"') do set PID=%%i
